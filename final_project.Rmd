...
The observations in our data must be independent of one another. In this data, we can not be sure this is the case, for example,  the avocados can be counted when multiple units are in a bag. Although it was told that the retail scan data comes directly from retailers’ cash registers based on actual retail sales of Hass avocados, we believe that we are not given enough information to verify this condition has been met.
...
```
From the histogram above, this model does meet the normality of residuals condition, as it appears to be a (roughly) normal distribution.
...
```
From both graphs, we observe that the spread of the residuals increases as the value of total volume increases. This does not fulfill the condition since there is not equal variance across all values of the explanatory variable, total volume. Thus, any inference for regression based on this model is not valid because it yields such a pattern. 
