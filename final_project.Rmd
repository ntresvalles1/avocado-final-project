...
1. Context: According to the USDA, the United States has tripled their per capita consumption of avocados  since the early 2000’s, with about 95% being Hass avocados. Along with its taste, size, and longer shelf life, Hass avocados have had greater growing yield in certain regions. The following dataset’s information directly comes from retailers describing the price of avocados and the amount they sell. 
...
avocado_mod_categorical <- lm(AveragePrice ~ type, data = avocado_data)
...
```{r, fig.cap = "Figure 1: TRENDS IN AVOCADO PRICES IN 2016", fig.align = "center"}
avocado_data_2016 <- avocado_data %>% 
  filter(year == 2016)

# Insert code to visualize the distribution of outcome variable:
ggplot(avocado_data_2016, aes(y = AveragePrice, x = Date)) +
  geom_point() +
